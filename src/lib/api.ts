  ...
  try {
    if (contentType.includes('application/json')) {
      ...
      if (data && typeof data === 'object' && 'message' in data) {
        ...
        if (typeof messageValue === 'string') return { message: messageValue, status }
        if (Array.isArray(messageValue)) return { message: messageValue.join('\n'), status }
      }
...
export async function login(email: string, password: string) {
  if (USE_MOCK) {
    await sleep(450)
    if (!email.includes('@')) throw { message: 'メールアドレスの形式が不正です' } satisfies ApiError
    if (!password) throw { message: 'パスワードを入力してください' } satisfies ApiError
    if (password.length < 8)
      throw { message: 'パスワードは8文字以上にしてください' } satisfies ApiError
